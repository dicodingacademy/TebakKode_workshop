...
class Webhook extends CI_Controller {
  ...
  private function followCallback($event)
  {
    ...
    if ($res->isSucceeded()) 
    {
      ...
      $stickerMessageBuilder = new StickerMessageBuilder(1, 15); // thumbs sticker
      $this->bot->pushMessage($event['source']['userId'], $stickerMessageBuilder);

      $message = "Haai, salam kenal, " . $profile['displayName'] . "!\n";
      $textMessageBuilder = new TextMessageBuilder($message);
      $this->bot->pushMessage($event['source']['userId'], $textMessageBuilder);
      
      $message = "Serius Kamu udah jago ngoding PHP? Gw tes yaa! \nKetik \"MULAI\" untuk mulai menjawab soal.";
      $textMessageBuilder = new TextMessageBuilder($message);
      $this->bot->pushMessage($event['source']['userId'], $textMessageBuilder);
